...
count_homhetrefetc <- function(cm_f, n_samples){
  ...
  cm_f_gt_only = data.frame(cm_f[,samples_start_column:dim(cm_f)[2]])
...
##Hufsah 28.1.2022
###adding a function to add filters in case we have only one sample
apply_filter_onesample <- function(cm_f, samples){
  cm_f$gt_events = NA
  cols_to_add = c('lowconf','noreads','FP','alwayscomplex','MISO','Mendelfail', 'INVDUP')
  cm_f$verdict = 'UNK'
  bincutoff = 5 # For 'lowconf'
  ...
  # Apply verdicts based on sample GTs
  cm_f$lowconf = cm_f$valid_bins <= bincutoff
  cm_f$noreads = (cm_f[,samples]=='noreads')
